...
EXTRACT_KEYWORD_SYSTEM_PROMPT = "너는 텍스트에서 하나의 키워드를 추출하는 역할을 할거야. 이 키워드는 구글에서 뉴스를 검색하는 용도로 사용할거야. 예를 들어서 [삼성전자 종가 기준 10년 그래프를 그려줘] 라는 사용자 입력이 있을 때, 여기서 '삼성전자'를 추출해줘야 해. 즉, 기업명을 추출해줘"
